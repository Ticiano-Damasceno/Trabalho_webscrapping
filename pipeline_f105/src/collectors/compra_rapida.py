"""
Coletor: Compra Rápida.

STATUS: esqueleto, não validado.
Pendência: inspecionar estrutura das páginas; nenhuma API pública adequada
foi confirmada até o momento (ver README).
"""

import requests
from .base import Oferta, RateLimiter, ColetorNaoImplementado

NOME_LOJA = "Compra Rápida"
BASE_URL = "https://www.comprarapida.example"  # TODO: confirmar domínio real
...
def _buscar_pagina(termo_busca: str) -> str:
    _rate_limiter.wait()
    url_busca = f"{BASE_URL}/busca?q={requests.utils.quote(termo_busca)}"
...
def _parsear_cards(html: str, termo_busca: str) -> list[Oferta]:
    raise ColetorNaoImplementado(
        "Loja ainda não inspecionada; domínio/seletores não confirmados."
    )
